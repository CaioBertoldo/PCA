# PCA
/*Faça uma função para multiplicar duas matrizes. A função recebe como parâmetro as três matrizes e as dimensões das duas primeiras matrizes e verificar se
 as matrizes podem ser multiplicadas e apresentar uma mensagem de erro, caso não seja possível. O resultado da multiplicação das duas primeiras matrizes deve 
 ser armazenado na terceira matriz. A função deve retornar falso se não for possível multiplicar as matrizes, e verdadeiro caso contrário.
